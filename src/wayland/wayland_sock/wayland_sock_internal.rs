use {
    std::{
        ...
        io::{
            ...
            Read,
            Write}},
    crate::wayland_sock::WaylandSockMsg};

pub struct WaylandSockInternal{
    stream: Option<UnixStream>
}

impl WaylandSockInternal{
    // ***** Public Functions *****
    ...
    // ***** Init Struct *****
    pub fn new() -> WaylandSockInternal{
        ...
        match var("XDG_RUNTIME_DIR"){
            ...
            Err(_) => return WaylandSockInternal{stream: None}
        }
        ...
        match UnixStream::connect(runtime_dir + "/" + &wayland_display) {
            Ok(sock) => {
                ...
                match sock.set_nonblocking(true){
                    Ok(_) => WaylandSockInternal{stream: Some(sock)},
                    Err(_) => WaylandSockInternal{stream: None}
                }
            }, Err(_) => WaylandSockInternal{
                stream: None
            }
        }
    }
...
impl Drop for WaylandSockInternal{
    fn drop(&mut self){
        match self.stream{
            Some(ref mut stream) => {
